...
def initialsetting(VF):
    ...
    time.sleep(0.3)
    rr.PropertySet2(0, 4, -6)    #Exposure value
    time.sleep(0.3)
    rr.PropertySet2(0, 6, 0)    #focus value
    time.sleep(0.3)
    rr.PropertySet2(1, 9, 50)    #Gain value
    time.sleep(0.3)
...
def coarse_MTF():
    ...
    while True:
        ...
        if IsimageOK==111:
            ...
            if motorGoF==0:
                ...
                ls=reg2.RunCmd("videoxu read f")        #low byte VCM_cal data
                AF_num=ShowReturnListHex(ls)
                AF_num_low=AF_num[6:8]
                AF_num_high=AF_num[8:10]
                AF_num_value1=int('0x'+AF_num_high,0)*256
                AF_num_value2=int('0x'+AF_num_low,0)
                AF_num_value=int('0x'+AF_num_high,0)*256+int('0x'+AF_num_low,0)

                print(MTF_far,VCM_pos,Drive_mode,AF_num_value)
                VCM_MTF[p]=MTF_far
                AF_Stat[p]=AF_num_value
                VCM_pos=VCM_pos+5
                p=p+1
                motorGoF=1
            ...
            #p=p+1
            #print(p)
            if VCM_pos>105:          # drive VCM 30 step
                print(int(time.time())-time_start)
                break

    motorstatus=0
    for k in range(1,50):
        if AF_Stat_Max<=AF_Stat[k]:
            VCM_MTF_Max=VCM_MTF[k]
            AF_Stat_Max=AF_Stat[k]
            VCM_Peak_pos=k*5
            if VCM_Peak_pos<31:
    ...
    VCM_cal_data1=hex((int(Drive_mode))-211 & (2**16-1))
    #print(VCM_MTF_Max,VCM_Peak_pos,Drive_mode,VCM_cal_data[4:6],VCM_cal_data[2:4])
    print("VCResult:{0} ;{1} ;{2}".format(VCM_MTF_Max,VCM_Peak_pos,VCM_cal_data1))
    time.sleep(1)

    reg2=rr.CLVRegCmd()                         #initial Lvreg
    ls=reg2.RunCmd("testxu write a 8")          #Enter TEDmode for EEprom access

    VCM_cal_data=VCM_cal_data1[2:len(VCM_cal_data1)]        #write VCM_cal into EEprom
    #print(len(VCM_cal_data))

    reg2=rr.CLVRegCmd()
    if len(VCM_cal_data)==1:
        #print(VCM_cal_data[0:len(VCM_cal_data)])
        ls=reg2.RunCmd("eeprom write 7d "+VCM_cal_data[0:len(VCM_cal_data)])
        ls=reg2.RunCmd("eeprom write 7e 0")
    elif len(VCM_cal_data)==4:
       # print(VCM_cal_data[2:len(VCM_cal_data)])
        #print(VCM_cal_data[0:len(VCM_cal_data)-2])
        ls=reg2.RunCmd("eeprom write 7d "+VCM_cal_data[2:len(VCM_cal_data)])
        ls=reg2.RunCmd("eeprom write 7e "+VCM_cal_data[0:len(VCM_cal_data)-2])
    elif len(VCM_cal_data)==2:
        #print(VCM_cal_data[0:len(VCM_cal_data)])
        ls=reg2.RunCmd("eeprom write 7d "+VCM_cal_data[0:len(VCM_cal_data)])
        ls=reg2.RunCmd("eeprom write 7e 0")
