...
def PicProcessing_MTF():
    ...
    ROI_width=250
    ROI_height=1
    image = cv2.imread("C:\\D\\UTS2.0_VC_ProductionLine\\VC_Dali2\\image2.BMP")#读取图像
    ...
    print("VCResult:{0} ;{1} ;{2}".format(MTF_ave,123,456))
#==============================================================================================================    
...
def Pixelvalue(image,ROIX,ROIY,ROI_width):
    ...
    j=0
    #print("VCResult:{0}".format(8))
    MTF=0
    for k in range(2,ROI_width-8):
        if Y[k-1]<Y[k] and Y[k-2]<Y[k] and Y[k+1]<Y[k] and Y[k+2]<Y[k]:# and Y[k]<230 and Y[k]>30: #max white under 210 and over 30
            ...
            maxpeak=max(Y[k+5], Y[k+6], Y[k+7])
            if minpeak<100 and maxpeak>150:
                tempMin[j]=minpeak
...
def Stream():
    ...
    while True:
        ...
        if p>3100 or IsimageOK==111:
            cf.VideoFrameSave("image.bmp")
            break
