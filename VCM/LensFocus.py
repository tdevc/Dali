    ...
    while True:
        ...
        if IsimageOK==111:
            ...
            if MTF_far+MTF_near==0:
                MTF_far=1
            Delta=round((MTF_far-MTF_near)/(MTF_far+MTF_near)*200,2)
            ...
            if int(time.time())-time_start>20 or MTF_near>30 or MTF_far>30:
                if motorGoF==0 and coarse_focus_flag==1:
                    print(int(time.time())-time_start)
                    coarse_focus_flag=0
            
            if motorGoF==0 and MTF_near<30 and coarse_focus_flag==1:
                motorGoF=1

            if int(time.time())-time_start>40 or abs(Delta)<5:   #Fine focus
                if motorGoF==0:
                    print(int(time.time())-time_start)
