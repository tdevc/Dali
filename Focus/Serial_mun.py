...
if value==0 or value==255:              #if there is no SN and create new
    ...
    mun=int(str(localtime.tm_year)[2:4])    #+128    #128(1000 0000) for 2nd station
    mun2=str(hex(mun))
...
ls=reg2.RunCmd("EEPROM write 56 1")        #forr Black version
...
ls=reg2.RunCmd("eeprom write 56 1")        #read Black version
ls=reg2.RunCmd("eeprom read 56")        #read test block
Black_version=int(ShowReturnList(ls))                #get Black version value
        
print("VCResult:{0};{1};{2}".format(serial_mun,SN,Black_version))    #sent serial number to log file
...
print(uuid1[4:6])
print(uuid1[6:8])'''
ls=reg2.RunCmd("videoxu write 7 0") #EIS
